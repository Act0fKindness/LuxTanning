    ...
    public function up(): void
    {
        Schema::table('jobs', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('jobs', 'actual_minutes')) {
                $table->integer('actual_minutes')->nullable()->after('completed_at');
            }
            if (!Schema::hasColumn('jobs', 'cancelled_at')) {
                $table->timestamp('cancelled_at')->nullable()->after('actual_minutes');
            }
            if (!Schema::hasColumn('jobs', 'last_lat')) {
                $table->decimal('last_lat', 10, 7)->nullable()->after('cancelled_at');
            }
